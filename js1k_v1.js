...
function get3DGradientsAt (tileIndex, triangleFactor) {
...
function processPixel (xin, yin, zin) {
    ...
    var cornerGradients = get3DGradientsAt(tileOrigin, triangleFactor);
...
function draw () {
    for (var x = 0; x < w; x++) {
        for (var y = 0; y < h; y++) {
            var idx = (x + y * w) * 4;
            img.data[idx+0] = img.data[idx+1] = img.data[idx+2] = (processPixel(x*.02,y*.02,time*.01)+1)*128;
            img.data[idx+3] = 255;
        }
    }
